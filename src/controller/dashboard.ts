...
export class DashboardController {
  ...
  obter(req: Request, res: Response): void {
    ...
    if (!utente) { res.status(404).json({ erro: 'Utente não encontrado.' }); return; }
    ...
    const limiar = db.prepare('SELECT score_minimo FROM limiares_alerta LIMIT 1').get() as any;
    ...
    // Medicação ativa
    const medicacaoAtiva = db.prepare(
      "SELECT * FROM medicacao WHERE utente_id=? AND estado='ATIVA' ORDER BY data_inicio DESC"
    ).all(utenteId);

    registarAuditoria(user.id, 'VER_DASHBOARD', 'utentes', utenteId as string);
